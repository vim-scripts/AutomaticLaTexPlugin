...
endtry
" }}}1
" {{{1 atplib#complet#OmniCometion
function! atplib#complete#OmniComplete(findstart, base)
    ...
    let s:append	= append
    let s:atp_completion_variables = deepcopy([
		\ pos, pos_saved, copy(line), copy(nchar),
		\ l, n, m, o, s, p, r, c, a, nr, color_nr,
		\ abegin, begin, color_begin, cbegin, tbegin,
		\ obegin, ebegin, pline, ppline, limit_line
		\ ])
    let g:atp_completion_variables = deepcopy(s:atp_completion_variables)

    if g:atp_debugTabCompletion
	let g:nchar	= nchar
	call atplib#Log("OmniCompletion.log", "nchar=".nchar)
	let g:l		= l
	call atplib#Log("OmniCompletion.log", "l=".l)
	let g:n		= n
	call atplib#Log("OmniCompletion.log", "n=".n)
	let g:o		= o
	call atplib#Log("OmniCompletion.log", "o=".o)
	let g:s		= s
	call atplib#Log("OmniCompletion.log", "s=".s)
	let g:p		= p
	call atplib#Log("OmniCompletion.log", "p=".p)
	let g:a		= a
	call atplib#Log("OmniCompletion.log", "a=".a)
	let g:nr	= nr
	call atplib#Log("OmniCompletion.log", "nr=".nr)

	let g:line	= line    
	call atplib#Log("OmniCompletion.log", "line=".line)
	let g:abegin	= abegin
	call atplib#Log("OmniCompletion.log", "abegin=".abegin)
	let g:tbegin	= tbegin
	call atplib#Log("OmniCompletion.log", "tbegin=".tbegin)
	let g:cbegin	= cbegin
	call atplib#Log("OmniCompletion.log", "cbegin=".cbegin)
	let g:obegin	= obegin
	call atplib#Log("OmniCompletion.log", "obegin=".obegin)
	let g:begin	= begin 
	call atplib#Log("OmniCompletion.log", "begin=".begin)
	let g:ebegin	= ebegin 
	call atplib#Log("OmniCompletion.log", "ebegin=".ebegin)
	let g:pline	= pline
	call atplib#Log("OmniCompletion.log", "pline=".pline)
	let g:ppline	= ppline
	call atplib#Log("OmniCompletion.log", "ppline=".ppline)
	let g:color_begin	= color_begin
	call atplib#Log("OmniCompletion.log", "color_begin=".color_begin)

	let g:limit_line= limit_line
	call atplib#Log("OmniCompletion.log", "limit_line=".limit_line)
    endif
...
" {{{2 SET COMPLETION METHOD
	...
	" but matches "" and "\\" (i.e. when completing "\" or "\\\" [end line
	" + command].
	    let s:completion_method='command'
	    " DEBUG:
	    let b:comp_method='command'
	    call atplib#Log("OmniCompletion.log", "b:comp_method=".b:comp_method)
	    let column = nr+2
	    let s:column = column
	    let s:append = append
	    set omnifunc=atplib#complete#OmniCompleteCommands
	    return nr
    "{{{3 --------- environment options
    elseif (l =~ '\\begin\s*{[^}]*}\s*\[[^\]]*$' && !normal_mode) &&
		\ index(g:atp_completion_active_modes, 'environment options') != -1 
	    let s:completion_method='environment options'
	    " DEBUG:
	    let b:comp_method='environment options'
	    call atplib#Log("OmniCompletion.log", "b:comp_method=".b:comp_method)
	    set omnifunc=atplib#complete#OmniCompleteEnvOptions
	    let s:column = nr+1
	    return nr+1
    "{{{3 --------- environment names
    elseif (pline =~ '\%(\\begin\|\\end\)\s*$' && begin !~ '}.*$' && !normal_mode) &&
		\ index(g:atp_completion_active_modes, 'environment names') != -1 
	    let s:completion_method='environment_names'
	    " DEBUG:
	    let b:comp_method='environment_names'
	    call atplib#Log("OmniCompletion.log", "b:comp_method=".b:comp_method)
	    set omnifunc=atplib#complete#OmniCompleteEnvNames
	    let s:column = nr+1
	    return nr+1
    "{{{3 --------- label
    elseif l =~ '\\\%(eq\)\?ref{[^}]*$' && !normal_mode &&
		\ index(g:atp_completion_active_modes, 'labels') != -1 
	    let s:completion_method='labels'
	    " DEBUG:
	    let b:comp_method='labels'
	    call atplib#Log("OmniCompletion.log", "b:comp_method=".b:comp_method)
	    set omnifunc=atplib#complete#OmniCompleteLabels
	    let col=match(l, '\%(.\|\\\%(eq\)\=ref\)*\\\(eq\)\=ref\s*{\zs\S*$')+1
	    let s:column = col-1
	    return col-1
    "{{{3 --------- pagestyle
    elseif l =~ '\\\%(pagestyle\|thispagestyle\){[^}]*$' &&
		\ index(g:atp_completion_active_modes, 'page styles') != -1 
	let s:completion_method='pagestyle'
	" DEBUG:
	let b:comp_method='pagestyle'
	call atplib#Log("OmniCompletion.log", "b:comp_method=".b:comp_method)
	set omnifunc=atplib#complete#OmniCompletePageStyle
	let s:column = nr+1
	return nr+1
    "{{{3 --------- pagenumbering
    elseif l =~ '\\pagenumbering{[^}]*$' &&
		\ index(g:atp_completion_active_modes, 'page numberings') != -1 
	let s:completion_method='pagenumbering'
	" DEBUG:
	let b:comp_method='pagenumbering'
	call atplib#Log("OmniCompletion.log", "b:comp_method=".b:comp_method)
	set omnifunc=atplib#complete#OmniCompletePageNumbering
	let s:column = nr+1
	return nr+1
    "{{{3 --------- bibitems
    elseif ppline =~ '\\\%(no\)\?[cC]ite\%(\%(al\)\?[tp]\*\?\|text\|num\|author\*\?\|year\%(par\)\?\)\?\(\s*\[[^]]*\]\s*\)\={[^}]*$' && !normal_mode &&
		\ index(g:atp_completion_active_modes, 'bibitems') != -1
	    let s:completion_method='bibitems'
	    " DEBUG:
	    let b:comp_method='bibitems'
	    call atplib#Log("OmniCompletion.log", "b:comp_method=".b:comp_method)
	    set omnifunc=atplib#complete#OmniCompleteBibItems
	    let time_bibitems=reltime()
	    let col = col('.') - 1
	    while col > 0 && line[col - 1] !~ '{\|,'
		    let col -= 1
	...
	if begin =~ '^color='
	    " This is for tikz picture color completion.
	    let s:completion_method='tikzpicture colors'
	    let b:comp_method='tikzpicture colors'
	    call atplib#Log("OmniCompletion.log", "b:comp_method=".b:comp_method)
	    set omnifunc=atplib#complete#OmniCompleteTikzColors
	    let s:column = color_nr+1
	    return color_nr+1
	"{{{4 ----------- tikzpicture keywords
	elseif l =~ '\%(\s\|\[\|{\|}\|,\|\.\|=\|:\)' . tbegin . '$' 
		    \ && !expert_mode
		let b:comp_method='tikzpicture keywords'
		call atplib#Log("OmniCompletion.log", "b:comp_method=".b:comp_method)
		let s:completion_method="tikzpicture keywords"
		set omnifunc=atplib#complete#OmniCompleteTikzKeywords
		let t=match(l,'\zs\<\w*$')
		" in case '\zs\<\w*$ is empty
		if t == -1
		    let t=col(".")
	...
	"{{{4 ----------- brackets
	    ...
	    let begParen = atplib#complete#CheckBracket(g:atp_bracket_dict)
	    let s:begParen = begParen
	    if begParen[1] != 0 || atplib#complete#CheckSyntaxGroups(['texMathZoneX', 'texMathZoneY']) &&
		    \ (!normal_mode &&  index(g:atp_completion_active_modes, 'brackets') != -1 ) ||
		    \ (normal_mode && index(g:atp_completion_active_modes_normal_mode, 'brackets') != -1 )

		let b:comp_method='brackets tikzpicture'
		let s:completion_method = 'brackets'
		set omnifunc=atplib#complete#OmniCompleteBrackets
		return col(".")
	"{{{4 ----------- close environments
	    elseif (!normal_mode &&  index(g:atp_completion_active_modes, 'close environments') != '-1' ) ||
			\ (normal_mode && index(g:atp_completion_active_modes_normal_mode, 'close environments') != '-1' )
		let s:completion_method='close_env'
		" DEBUG:
		let b:comp_method='close_env tikzpicture' 
		call atplib#Log("OmniCompletion.log", "b:comp_method=".b:comp_method)
		set omnifunc=atplib#complete#OmniCompleteCloseEnv
		return col(".")
	    else
		return -1
	    endif
    ...
    elseif l =~ '\\usepackage\[[^\]]*=\%([^\],]*\|{\([^}]\+,\)\?[^}]*\)$' &&
		...
		\ !normal_mode &&
		\  index(g:atp_completion_active_modes, 'package options values') != -1
	    let s:completion_method='package options values'
	    let b:comp_method=s:completion_method
	    let col=len(matchstr(l,'\\\%(documentclass\|usepackage\)\[.*=\%({[^}]*,\|{\)\?\ze'))
	    if col==0
		let col=len(matchstr(l,'\\\%(documentclass\|usepackage\)\[\ze'))
	    endif
	    set omnifunc=atplib#complete#OmniCompletePackageOptionsValues
	    let s:column = col
	    return col
    "{{{3 --------- package options
    elseif l =~ '\\usepackage\[[^\]]*$' && !normal_mode &&
		\  index(g:atp_completion_active_modes, 'package options') != -1
	    let s:completion_method='package options'
	    let b:comp_method=s:completion_method
	    set omnifunc=atplib#complete#OmniCompletePackageOptions
	    let col=len(matchstr(l,'^.*\\\%(documentclass\|usepackage\)\[.*,\ze'))
	    if col==0
		let col=len(matchstr(l,'^.*\\\%(documentclass\|usepackage\)\[\ze'))
	    endif
    ...
    elseif pline =~ '\\usepackage\%([.*]\)\?\s*' && !normal_mode &&
		\  index(g:atp_completion_active_modes, 'package names') != -1
	    let s:completion_method='package'
	    " DEBUG:
	    let b:comp_method='package'
	    call atplib#Log("OmniCompletion.log", "b:comp_method=".b:comp_method)
	    set omnifunc=atplib#complete#OmniCompletePackages
	    let s:column = nr+1
	    return nr+1
    "{{{3 --------- tikz libraries
    elseif pline =~ '\\usetikzlibrary\%([.*]\)\?\s*' && !normal_mode &&
		\ index(g:atp_completion_active_modes, 'tikz libraries') != -1
	    let s:completion_method='tikz libraries'
	    " DEBUG:
	    let b:comp_method='tikz libraries'
	    call atplib#Log("OmniCompletion.log", "b:comp_method=".b:comp_method)
	    set omnifunc=atplib#complete#OmniCompleteTikzLibraries
	    let s:column = nr+1
	    return nr+1
    "{{{3 --------- inputfiles
    elseif (l =~ '\\input\%([^{}]*\|\s*{[^}]*\)$'||
	  ...
	  \ index(g:atp_completion_active_modes, 'input files') != -1
	    ...
	    let s:completion_method='inputfiles'
	    " DEBUG:
	    let b:comp_method='inputfiles'
	    call atplib#Log("OmniCompletion.log", "b:comp_method=".b:comp_method)
	    set omnifunc=atplib#complete#OmniCompleteInputFiles
	    let s:column = nr+1
	    return nr+1
    "{{{3 --------- includegraphics
    elseif (l =~ '\\includegraphics\s*\(\[[^\]]*\]\s*\)\?{[^}]*$') &&
		\ index(g:atp_completion_active_modes, 'includegraphics') != -1
	    let s:completion_method='includegraphics'
	    " DEBUG:
	    let b:comp_method='includegraphics'
	    call atplib#Log("OmniCompletion.log", "b:comp_method=".b:comp_method)
	    set omnifunc=atplib#complete#OmniCompleteIncludeGraphics
	    let s:column = nr+1
	    return nr+1
    "{{{3 --------- bibfiles
    elseif pline =~ '\\\%(bibliography\%(style\)\@!\|addbibresource\|addglobalbib\)' && !normal_mode &&
		\ index(g:atp_completion_active_modes, 'bibfiles') != -1
	    let s:completion_method='bibfiles'
	    " DEBUG:
	    let b:comp_method='bibfiles'
	    call atplib#Log("OmniCompletion.log", "b:comp_method=".b:comp_method)
	    set omnifunc=atplib#complete#OmniCompleteBibFiles
	    let s:column = nr+1
	    return nr+1
    "{{{3 --------- bibstyles
    elseif pline =~ '\\bibliographystyle' && !normal_mode  &&
	 \ index(g:atp_completion_active_modes, 'bibstyles') != -1
	    let s:completion_method='bibstyles'
	    let b:comp_method='bibstyles'
	    call atplib#Log("OmniCompletion.log", "b:comp_method=".b:comp_method)
	    set omnifunc=atplib#complete#OmniCompleteBibStyles
	    let s:column = nr+1
	    return nr+1
    "{{{3 --------- todo & missingfigure options
    elseif obegin =~ '\\todo\[[^\]]*$' &&
		\ ( index(g:atp_completion_active_modes, 'todonotes') != -1 ) 
	    let s:completion_method='todo options'
	    let b:comp_method='todo options'
	    call atplib#Log("OmniCompletion.log", "b:comp_method=".b:comp_method)
	    set omnifunc=atplib#complete#OmniCompleteTodoOptions
	    let s:column = nr+1
	    return nr+1
    elseif obegin =~ '\\missingfigure\[[^\]]*$' &&
		\ ( index(g:atp_completion_active_modes, 'todonotes') != -1 )
	    let s:completion_method='missingfigure options'
	    let b:comp_method='missingfigure options'
	    call atplib#Log("OmniCompletion.log", "b:comp_method=".b:comp_method)
	    set omnifunc=atplib#complete#OmniCompleteMissingFigure
	    return nr+1
    "{{{3 --------- documentclass options
    elseif l =~ '\\documentclass\s*\[[^\]]*$' && !normal_mode  &&
	    \ index(g:atp_completion_active_modes, 'documentclass options') != -1
	    let s:completion_method='documentclass options'
	    let b:comp_method=s:completion_method
	    call atplib#Log("OmniCompletion.log", "b:comp_method=".b:comp_method)
	    let col=len(matchstr(l,'^.*\\\%(documentclass\|usepackage\)\[.*,\ze'))
	    if col==0
	    ...
	    set omnifunc=atplib#complete#OmniCompleteDocumentClassOptions
	    let s:column = col
	    return col
    "{{{3 --------- documentclass
    elseif pline =~ '\\documentclass\>' && !normal_mode  &&
		\ index(g:atp_completion_active_modes, 'documentclass') != -1
	    let s:completion_method='documentclass'
	    let b:comp_method='documentclass'
	    call atplib#Log("OmniCompletion.log", "b:comp_method=".b:comp_method)
	    call atplib#Log("OmniCompletion.log", "b:comp_method=".b:comp_method)
	    set omnifunc=atplib#complete#OmniCompleteDocumentClass
	    let s:column = nr+1
	    return nr+1
    "{{{3 --------- font family
    elseif l =~ '\%(\\renewcommand\s*{\s*\\\%(rm\|sf\|bf\|tt\|md\|it\|sl\|sc\|up\)default\s*}\s*{\|\\usefont{[^}]*}{\|\\DeclareFixedFont{[^}]*}{[^}]*}{\|\\fontfamily{\)[^}]*$' && !normal_mode  &&
		\ index(g:atp_completion_active_modes, 'font family') != -1
	    let s:completion_method='font family'
	    let b:comp_method='font family'
	    call atplib#Log("OmniCompletion.log", "b:comp_method=".b:comp_method)
	    set omnifunc=atplib#complete#OmniCompleteFontFamily
	    let s:column = nr+1
	    return nr+1
    "{{{3 --------- font series
    elseif l =~ '\%(\\usefont{[^}]*}{[^}]*}{\|\\DeclareFixedFont{[^}]*}{[^}]*}{[^}]*}{\|\\fontseries{\)[^}]*$' && 
		\ !normal_mode  &&
		\ index(g:atp_completion_active_modes, 'font series') != -1
	    let s:completion_method='font series'
	    let b:comp_method='font series'
	    call atplib#Log("OmniCompletion.log", "b:comp_method=".b:comp_method)
	    set omnifunc=atplib#complete#OmniCompleteFontSeries
	    let s:column = nr+1
	    return nr+1
    "{{{3 --------- font shape
    elseif l =~ '\%(\\usefont{[^}]*}{[^}]*}{[^}]*}{\|\\DeclareFixedFont{[^}]*}{[^}]*}{[^}]*}{[^}]*}{\|\\fontshape{\)[^}]*$' 
		\ && !normal_mode  &&
		\ index(g:atp_completion_active_modes, 'font shape') != -1
	    let s:completion_method='font shape'
	    let b:comp_method='font shape'
	    call atplib#Log("OmniCompletion.log", "b:comp_method=".b:comp_method)
	    set omnifunc=atplib#complete#OmniCompleteFontShape
	    let s:column = nr+1
	    return nr+1
    "{{{3 --------- font encoding
    elseif l =~ '\%(\\usefont{\|\\DeclareFixedFont{[^}]*}{\|\\fontencoding{\)[^}]*$' && !normal_mode  &&
		\ index(g:atp_completion_active_modes, 'font encoding') != -1
	    let s:completion_method='font encoding'
	    let b:comp_method='font encoding'
	    call atplib#Log("OmniCompletion.log", "b:comp_method=".b:comp_method)
	    set omnifunc=atplib#complete#OmniCompleteFontEncoding
	    let s:column = nr+1
	    return nr+1
    "{{{3 --------- command values
    " this is at the end because there are many command completions done
    ...
    elseif (l =~ '\%(\\\w\+\({\%([^}]\|{[^}]*}\)*\)\?{\%([^}]\|{[^}]*}\)*$\|\\renewcommand{[^}]*}{[^}]*$\)' && !normal_mode) &&
		\ index(g:atp_completion_active_modes, 'environment names') != -1 
	    let s:completion_method="command values"
	    " DEBUG:
	    let b:comp_method=s:completion_method
	    call atplib#Log("TabCompletion.log", "b:comp_method=".b:comp_method)
	    if l !~ '\\renewcommand{[^}]*}{[^}]*$'
	    ...
	    set omnifunc=atplib#complete#OmniCompleteCommandValues
	    let s:column = col
	    return col
    "{{{3 --------- brackets, algorithmic, abbreviations, close environments
    else
	...
	if l =~ '=[a-zA-Z]\+\*\=$' &&
		\ index(g:atp_completion_active_modes, 'abbreviations') != -1 &&
		\ !atplib#IsInMath() 
	    let s:completion_method='abbreviations' 
	    let b:comp_method='abbreviations'
	    call atplib#Log("TabCompletion.log", "b:comp_method=".b:comp_method)
	    set omnifunc=atplib#complete#OmniCompleteAbbreviations
	    let s:column = match(l, '^.*\zs=')
	    return s:column
	"{{{4 --------- brackets
	elseif begParen[2] != '\begin' && ( begParen[1] != 0 || atplib#complete#CheckSyntaxGroups(['texMathZoneX', 'texMathZoneY']) &&
		\ (!normal_mode &&  index(g:atp_completion_active_modes, 'brackets') != -1 ) ||
	    ...
	    let b:comp_method='brackets'
	    let s:begParen = begParen
" 	    let move = ( !expert_mode ? join(map(range(len(bracket)), '"\<Left>"'), '') : '' )
	    set omnifunc=atplib#complete#OmniCompleteBrackets
	    let s:column = col(".")
	    return col(".")
	"{{{4 --------- close environments
	elseif (!normal_mode &&  index(g:atp_completion_active_modes, 'close environments') != '-1' ) ||
		...
		let b:tc_return = "close_env math"
		let s:append = append
		set omnifunc=atplib#complete#OmniCompleteCloseMath
		let s:column = col(".")
		return col(".")
	    " Close environments
	    else
    ...
    let s:completion_method = ( exists("completion_method") ? completion_method : 'completion_method does not exists' )
    let b:completion_method = s:completion_method
"}}}2
" {{{2 close environments
    if s:completion_method=='close_env'
	let b:completion_method = s:completion_method
...
" Note cursor position when omnifunc is called for the second time is set
" where the completion starts, not where the cursor was standing.
" This affects l variable for example.
" {{{2 atplib#complete#OmniCompleteFinal
function! atplib#complete#OmniCompleteFinal(completions,matched_word)
    let completions = a:completions

    let completion_method = s:completion_method
    let append = s:append
    let [ pos, pos_saved, line, nchar,
	\ l, n, m, o, s, p, r, c, a, nr, color_nr,
	\ abegin, begin, color_begin, cbegin, tbegin,
	\ obegin, ebegin, pline, ppline, limit_line
	\ ] = s:atp_completion_variables
    let column = s:column
    let g:begin = begin
"     let g:cpos = getpos(".")
"     all cursor(line("."), 30)
"     let g:apos = getpos(".")
"     call cursor(g:cpos[1-2])
"     let g:line = getline(line("."))
	...
	if count(['command', 'tikzpicture commands', 'tikzpicture keywords', 'command values'], completion_method) && 
	    ...
	    " Check Brackets 
	    let b:comp_method   .= " brackets: 1"
	    let g:omni_pos	= s:omni_pos
	    let cl_return 	= atplib#complete#GetBracket(append,g:atp_bracket_dict, 0, [], s:omni_pos)
	    let g:cl_return 	= cl_return
	    ...
	    if atplib#complete#CheckClosed_math('texMathZoneV') || 
			...
			\ b:atp_TexFlavor == 'plaintex' && atplib#complete#CheckClosed_math('texMathZoneY')
		let zone = 'texMathZoneVWXY' 	" DEBUG
		call atplib#complete#CloseLastEnvironment(append, 'math', 0, [0,0], 1)
		...
		let zone	= env_name 	" DEBUG
		if env_opened != [0, 0]
		    return atplib#complete#CloseLastEnvironment('a', 'environment', env_name, env_opened, 1)
		endif
	    endif
	    " DEBUG
	    if exists("zone")
	...
	elseif len == 0 && 
		    \ completion_method != 'labels' && 
		    \ completion_method != 'bibitems' 
		    \ || len == 1 && get(completions, 0, "") == a:matched_word || 
		    \ completion_method != 'brackets' &&
		    \ completion_method != 'labels' &&
		    \ completion_method != 'bibitems' &&
		    \ completion_method != 'bibfiles' &&
...
" 		    \ completion_method == 'bibfiles'
	    let b:tc_return='close_bracket end'
	    let b:comp_method .= " brackets: 2"
	    return atplib#complete#GetBracket(append,g:atp_bracket_dict, 0, [], s:omni_pos)
	endif
    endif
    return ''
endfunction
" {{{2 atplib#complete#OmniCompleteCommands
try
function! atplib#complete#OmniCompleteCommands(findstart, base)

    set omnifunc=atplib#complete#OmniComplete
    let expert_mode	= 1
    let append		= s:append
    let [ pos, pos_saved, line, nchar,
	\ l, n, m, o, s, p, r, c, a, nr, color_nr,
...
" 	endif
	...
	" {{{4 -------------------- fancyhdr & makeidx Commands
"	if we are not in math mode or if we do not care about it or we are in non expert mode.
	if (!g:atp_MathOpened || !math_is_opened ) || expert_mode == 0
	    call extend(completion_list, g:atp_Commands)
	    " FANCYHDR
	    if atplib#search#SearchPackage('fancyhdr', stop_line)
		call extend(completion_list, g:atp_fancyhdr_commands)
...
" 	endif
	...
	call extend(completion_list, [ '\label{' . short_env_name ],0)
	let completions = filter(copy(completion_list), 'v:val =~# escape(a:base, ''\'')')
	if g:atp_completion_truncate && expert_mode
	    call filter(completions, 'len(substitute(v:val,"^\\","","")) >= g:atp_completion_truncate')
	endif

	call extend(completion_list, g:atp_greek_letters)
	let g:count = count(completion_list, cbegin)
	if count(completion_list, cbegin) >= 1
	    " Add here brackets - somebody might want to
	    " close a bracket after \nu and not get \numberwithin{ (which is
	    " rarely used).
	    let b:comp_method = "brackets in commands"
	    if index(g:atp_completion_active_modes, 'brackets') != -1  ||
		    \ index(g:atp_completion_active_modes_normal_mode, 'brackets') != -1
		let bracket=atplib#complete#GetBracket(append, g:atp_bracket_dict)
		if bracket != "0" && bracket != ""
		    let completions = extend([cbegin.bracket], completions)
		endif
	...
	if len(completions) == 0
	    return [a:base.atplib#complete#OmniCompleteFinal(completions,a:base)]
	elseif len(completions) == 1 && a:base == completions[0]
	    return [completions[0].atplib#complete#OmniCompleteFinal(completions,a:base)]
	else
	    return completions
	endif
endfunction 
...
endtry "}}}2
" {{{2 atplib#complete#OmniCompleteEnvOptions 
function! atplib#complete#OmniCompleteEnvOptions(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
    let expert_mode = 1
    ...
    let completions = filter(copy(completion_list), 'v:val =~ ''^''.a:base')
    if len(completions) == 0
	return [a:base.atplib#complete#OmniCompleteFinal(completions,a:base)]
    elseif len(completions) == 1 && a:base == completions[0]
	return [completions[0].atplib#complete#OmniCompleteFinal(completions,a:base)]
...
" {{{2 atplib#complete#OmniCompleteEnvNames 
function! atplib#complete#OmniCompleteEnvNames(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
    let expert_mode = 1
...
" 	endif
    ...
    let completions = filter(completion_list, 'v:val =~ ''^''.a:base')
    if len(completions) == 0
	return [a:base.atplib#complete#OmniCompleteFinal(completions,a:base)]
    elseif len(completions) == 1 && a:base == completions[0]
	return [completions[0].atplib#complete#OmniCompleteFinal(completions,a:base)]
...
" {{{2 atplib#complete#OmniCompleteLabels 
function! atplib#complete#OmniCompleteLabels(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
    let expert_mode = 1
    ...
    for data in aux_data
	" match label by string or number
	if ( data[0] =~ '^' . pattern || data[1] =~ '^'. pattern ) && expert_mode || ( data[0] =~ pattern || data[1] =~ pattern ) && !expert_mode
	    let close = ( nchar == '}' ? '' : '}' )
	    call add(completion_dict, { "word" : data[0].close, "abbr" : data[0], "menu" : ( data[2] == 'equation' && data[1] != "" ? "(".data[1].")" : data[1] ) , "kind" : data[2][0] })
	endif
    endfor 
...
" {{{2 atplib#complete#OmniCompletePageStyle 
function! atplib#complete#OmniCompletePageStyle(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
    let expert_mode = 1
...
" {{{2 atplib#complete#OmniCompletePageNumbering 
function! atplib#complete#OmniCompletePageNumbering(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
    let expert_mode = 1
...
" {{{2 atplib#complete#OmniCompleteBibItems 
function! atplib#complete#OmniCompleteBibItems(findstart, base)
    let pos		= getpos(".")
    let [ pos, pos_saved, line, nchar,
	\ l, n, m, o, s, p, r, c, a, nr, color_nr,
	\ abegin, begin, color_begin, cbegin, tbegin,
...
" {{{2 atplib#complete#OmniCompleteTikzColors 
function! atplib#complete#OmniCompleteTikzColors(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
    let expert_mode = 1
...
" {{{2 atplib#complete#OmniCompleteTikzKeywords 
function! atplib#complete#OmniCompleteTikzKeywords(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
    let expert_mode = 1
...
" {{{2 atplib#complete#OmniCompleteTikzCommands
function! atplib#complete#OmniCompleteTikzCommands(findstart, base)
...
" {{{2 atplib#complete#OmniCompleteBrackets 
function! atplib#complete#OmniCompleteBrackets(findstart, base)
    " It is used inside tikzpicture environment and brackets type completion.
    set omnifunc=atplib#complete#OmniComplete
    let bracket=atplib#complete#GetBracket('i', g:atp_bracket_dict, 0, s:begParen, s:omni_pos)
    unlet s:begParen
"     let move = ( !expert_mode ? join(map(range(len(bracket)), '"\<Left>"'), '') : '' )
    return [bracket]
endfunction
" {{{2 atplib#complete#OmniCompleteCloseEnv
function! atplib#complete#OmniCompleteCloseEnv()
    " Used inside tikzpicture environment.
    set omnifunc=atplib#complete#OmniComplete
    let append="i"
    if atplib#complete#CheckClosed_math('texMathZoneV') || 
	    \ atplib#complete#CheckClosed_math('texMathZoneW') ||
...
" {{{2 atplib#complete#OmniCompletePackageOptionsValues 
function! atplib#complete#OmniCompletePackageOptionsValues(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
    let [ pos, pos_saved, line, nchar,
	\ l, n, m, o, s, p, r, c, a, nr, color_nr,
	\ abegin, begin, color_begin, cbegin, tbegin,
...
" {{{2 atplib#complete#OmniCompletePackageOptions 
function! atplib#complete#OmniCompletePackageOptions(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
    let [ pos, pos_saved, line, nchar,
	\ l, n, m, o, s, p, r, c, a, nr, color_nr,
	\ abegin, begin, color_begin, cbegin, tbegin,
...
" {{{2 atplib#complete#OmniCompletePackages 
function! atplib#complete#OmniCompletePackages(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
    let expert_mode = 1
...
" {{{2 atplib#complete#OmniCompleteTikzLibraries 
function! atplib#complete#OmniCompleteTikzLibraries(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
...
" {{{2 atplib#complete#OmniCompleteInputFiles 
function! atplib#complete#OmniCompleteInputFiles(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
...
" {{{2 atplib#complete#OmniCompleteIncludeGraphics 
function! atplib#complete#OmniCompleteIncludeGraphics(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
    let [ pos, pos_saved, line, nchar,
	\ l, n, m, o, s, p, r, c, a, nr, color_nr,
	\ abegin, begin, color_begin, cbegin, tbegin,
...
" {{{2 atplib#complete#OmniCompleteBibFiles 
function! atplib#complete#OmniCompleteBibFiles(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
...
" {{{2 atplib#complete#OmniCompleteBibStyles 
function! atplib#complete#OmniCompleteBibStyles(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
...
" {{{2 atplib#complete#OmniCompleteTodoOptions 
function! atplib#complete#OmniCompleteTodoOptions(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
...
" {{{2 atplib#complete#OmniCompleteMissingFigure 
function! atplib#complete#OmniCompleteMissingFigure(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
...
" {{{2 atplib#complete#OmniCompleteDocumentClassOptions 
function! atplib#complete#OmniCompleteDocumentClassOptions(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
    let [ pos, pos_saved, line, nchar,
	\ l, n, m, o, s, p, r, c, a, nr, color_nr,
	\ abegin, begin, color_begin, cbegin, tbegin,
...
" {{{2 atplib#complete#OmniCompleteDocumentClass 
function! atplib#complete#OmniCompleteDocumentClass(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
    let expert_mode = 1
    let [ pos, pos_saved, line, nchar,
	\ l, n, m, o, s, p, r, c, a, nr, color_nr,
...
" {{{2 atplib#complete#OmniCompleteFontFamily 
function! atplib#complete#OmniCompleteFontFamily(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
    let [ pos, pos_saved, line, nchar,
	\ l, n, m, o, s, p, r, c, a, nr, color_nr,
	\ abegin, begin, color_begin, cbegin, tbegin,
...
" {{{2 atplib#complete#OmniCompleteFontSeries 
function! atplib#complete#OmniCompleteFontSeries(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
    let [ pos, pos_saved, line, nchar,
	\ l, n, m, o, s, p, r, c, a, nr, color_nr,
	\ abegin, begin, color_begin, cbegin, tbegin,
...
" {{{2 atplib#complete#OmniCompleteFontShape 
function! atplib#complete#OmniCompleteFontShape(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
    let [ pos, pos_saved, line, nchar,
	\ l, n, m, o, s, p, r, c, a, nr, color_nr,
	\ abegin, begin, color_begin, cbegin, tbegin,
...
" {{{2 atplib#complete#OmniCompleteFontEncoding 
function! atplib#complete#OmniCompleteFontEncoding(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
    let [ pos, pos_saved, line, nchar,
	\ l, n, m, o, s, p, r, c, a, nr, color_nr,
	\ abegin, begin, color_begin, cbegin, tbegin,
...
" {{{2 atplib#complete#OmniCompleteCommandValues 
function! atplib#complete#OmniCompleteCommandValues(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
    let expert_mode = 1
    let [ pos, pos_saved, line, nchar,
	\ l, n, m, o, s, p, r, c, a, nr, color_nr,
...
" {{{2 atplib#complete#OmniCompleteAbbreviations 
function! atplib#complete#OmniCompleteAbbreviations(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
    let expert_mode = 1
...
" {{{2 atplib#complete#OmniCompleteAlgorithmic 
function! atplib#complete#OmniCompleteAlgorithmic(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
    return [atplib#complete#CloseLastBracket(g:atp_algorithmic_dict, 1, 1)]
endfunction
" {{{2 atplib#complete#OmniCompleteCloseMath 
function! atplib#complete#OmniCompleteCloseMath(findstart, base)
    set omnifunc=atplib#complete#OmniComplete

    let append = s:append
    unlet s:append
    return [atplib#complete#CloseLastEnvironment(append, 'math', 0, [0,0], 1)]
endfunction
" {{{2 atplib#complete#OmniCompleteCloseEnvironment 
function! atplib#complete#OmniCompleteCloseEnvironment(findstart, base)
    set omnifunc=atplib#complete#OmniComplete
